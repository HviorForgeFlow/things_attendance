...
class AddSingleton(models.TransientModel):
    ...
    def button_add_clocking(self):
        self.ensure_one()
        # _logger.info( 'this is info:'+OKBLUE+ 'Button insert_timestamp works'+ENDC)
        ...
        if not self.employee_id:
            raise exceptions.UserError(_("Please select an Employee"))

        if not self.clocking_to_add_or_delete:
            raise exceptions.UserError(_("Please enter a time stamp."))
