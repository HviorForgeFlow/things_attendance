...
maxAllowedWorkingHours = (
    12  # max number of hours allowed between Check-In and Check-Out
)
...
class AttendanceHelpers:
    def __init__(
        ...
        self.source = (
            source or defaultClockingSource
        )  # in which device RAS2 (for example) was the clocking issued
    ...
    def is_timestamp_too_old(self):
        ...
        # self.cutting_date = datetime.strptime(datetime.now().strftime(DATETIME_FORMAT)
        # , '%Y-%m-%d') - relativedelta(days=self.howManyDaysAllowedToChange)
    ...
    def is_timestamp_already_registered(self, type_of_check):
        ...
        return self.attendanceModel.sudo().search(
            [
    ...
    def are_input_parameters_valid(self):
        ...
        if self.is_timestamp_too_old():
            self.warningMessage = (
                ...
                + str(self.howManyDaysAllowedToChange)
                + " days in the past.\n"
    ...
    def get_adjacent_clockings(self):
        self.attendances_check_in_sorted = (
            self.attendanceModel.sudo()
            .search(
                [
                    ("employee_id", "=", self.employee_id),
                    ("check_in", "!=", False),
        ...
        self.attendances_check_out_sorted = (
            self.attendanceModel.sudo()
            .search(
                [
                    ("employee_id", "=", self.employee_id),
        ...
        if self.PCI:
            self.PCI_exists = True
        else:
            self.PCI_exists = False

        if self.PCO:
            self.PCO_exists = True
        else:
            self.PCO_exists = False

        if self.NCI:
            self.NCI_exists = True
        else:
            self.NCI_exists = False

        if self.NCO:
            self.NCO_exists = True
        else:
            self.NCO_exists = False
    ...
    def could_register_using_only_PCI_and_PCO(self):
        if self.PCI_exists and not self.PCO_exists:
            if (
                self.register_new_Timestamp_in_existing_PCI_Attendance_Record_as_CheckOut()
            ):
                return True
    ...
    def register_new_Timestamp_in_existing_NCI_Attendance_Record_as_CheckIn(self):
        ...
        # _logger.debug("self.NCI.check_in: {}-  self.timestamp_dt: {}".format(
        #      self.NCI.check_in, self.timestamp_dt))
    ...
    def register_CheckIn_creating_a_new_Attendance_Record(self):
        ...
        self.attendanceModel.sudo().create(vals)
        self.prepare_exit_register_clocking_process()
    ...
    def could_register_using_NCI_and_NCO(self):
        if self.NCI_exists and not self.NCO_exists:
            if (
                self.register_new_Timestamp_in_existing_NCI_Attendance_Record_as_CheckIn()
            ):
                return True
        if not self.NCI_exists and self.NCO_exists:
            if (
                self.register_new_Timestamp_in_existing_NCO_Attendance_Record_as_CheckIn()
            ):
                return True
        if self.NCI_exists and self.NCO_exists:
            ...
            if (
                self.NCI == self.NCO
            ):  # doesn't need to check because it is the only possibility left
                if (
                    self.register_new_Timestamp_in_existing_attendance_record_with_NCI_AND_NCO()
                ):
                    return True
        return False
    ...
    def nextClockingIsASingleton(self, timestamp):
        self.nextCheckIn = self.attendanceModel.sudo().search(
            [
                ("employee_id", "=", self.employee_id),
                ("check_in", ">", timestamp),
            ],
