...
class HrEmployee(models.Model):
    ...
    def register_attendance_with_external_timestamp(self, timestamp=None):
        ...
        if not timestamp:
            ...
            _logger.debug(
                "in attendance_action_change_sync -"
                "got timestamp as an input from outside the method (call)",
                timestamp,
            )

        _logger.debug("in attendance_action_change_sync - timestamp ", timestamp)
        ...
        if self.attendance_state != "checked_in":
            ...
            _logger.debug(
                "in attendance_action_change_sync - when checked_out - self", self
            )
            _logger.debug(
                "in attendance_action_change_sync - when checked_out - self.attendance_state ",
            ...
            if attendance:
                _logger.debug(
                    "in attendance_action_change_sync - when checked_in - self", self
                )
                _logger.debug(
                    "in attendance_action_change_sync"
                    "- when checked_in - self.attendance_state ",
                    self.attendance_state,
                )
    ...
    def registerAttendanceWithExternalTimestamp(self, card_code, timestamp=None):
        ...
        if not timestamp:
            ...
            _logger.debug(
                "in register_attendance - got timestamp as an input"
                " from outside the method (call)",
                timestamp,
            )
        _logger.debug("in register_attendance - timestamp: ", timestamp)
        _logger.debug(
            "in register_attendance - fields.Datetime.to_datetime(time.strftime())",
            fields.Datetime.to_datetime(time.strftime("%Y-%m-%d %H:%M:%S")),
        )
        if employee:
        ...
        try:
            ...
            _logger.debug("in register_attendance - attendance: ", attendance)
            if attendance:
                ...
                if attendance.check_out:
                    ...
                    res["action"] = "check_in"
                _logger.debug("in register_attendance - res: ", res)
                return res
            else:
    ...
    def registerMultipleAsyncAttendances(self, attendancesToDispatch):
        ...
        for attendance in attendancesToDispatch:
            res = self.registerAttendanceWithExternalTimestamp(
                attendance[0], attendance[1]
            )
            if res["action"] == attendance[2]:
                _logger.debug(
                    "wonderful, there is concordance database and async checkinORcheckout"
                )
            else:
                _logger.debug("Oh! no concordance database and async checkinORcheckout")
    ...
    def get_attendance_information_of_all_employees(self, number_of_days=60):
        ...
        def is_attendance_to_be_sent(attendance):
            ...
            def is_attendance_too_old():
                cutting_date = attendance.check_in.fromtimestamp(
                    time.time() - (number_of_days * 24 * 60 * 60)
                )
        ...
        _logger.debug(json_attendance_information_of_all_employees)
