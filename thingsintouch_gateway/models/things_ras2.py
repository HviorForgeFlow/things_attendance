...
class ThingsRAS2(models.Model):
    ...
    RASxxx = fields.Char("Name", default="not defined", required=True)
    ...
    shouldGetFirmwareUpdate = fields.Boolean(
        "Update Firmware and Reboot (now)",
        required=True,
        default=False,
        help="Update the firmware after rebooting",
    )
    setRebootAt = fields.Datetime(
        "Reboot once the Terminal at this Time",
        help="Time when the Terminal will be rebooted",
        default=None,
    )
    shutdownTerminal = fields.Boolean(
        "Shutdown (Turn Off the Terminal)",
    ...
    updateAvailable = fields.Boolean(
        string="Firmware Update Available", required=True, default=False, readonly=True
    )
    lastConnectionOdooTerminal = fields.Datetime(
        "Last Connection",
        help="Timestamp of the last successful connection between the Device and Odoo",
        default=None,
        readonly=True,
    )
    minimumTimeBetweenClockings = fields.Integer(
        string="Minimum Time between Clockings [s]",
        required=True,
        default=300,
        help="How long is clocking again blocked [s]",
    )
    period_odoo_routine_check = fields.Integer(
        string="Time between Routine checks [s]",
        required=True,
        default=12,
    ...
    installedPythonModules = fields.Char(
        "Installed Python Modules",
        readonly=True,
        help="Installed Python Modules in the Terminal",
    )
    firmwareVersion = fields.Char("Firmware Version", readonly=True)
    lastFirmwareUpdateTime = fields.Datetime(
        "Last Firmware Update",
        help="Last Update of the Firmware of the Terminal",
        default=None,
        readonly=True,
    )
    lastTimeTerminalStarted = fields.Datetime(
        "Last Time Device Started", default=None, readonly=True
    )
    updateFailedCount = fields.Integer(
        "How Many Times the last Firmware Update Failed", readonly=True
    )
    ...
    card_registered = fields.Char(
        "Text Display for 'clocking registered'",
        default="Registered",
        help="Text to show on Display when card is registered for async clocking",
    )
    too_little_time_between_clockings = fields.Char(
        "Text Display for 'too little time between clockings'",
        default="Too Soon",
        help="Text to show on Display when card is not registered for async clocking"
        "because there was too little time between card swipes",
    )

    setup_password = fields.Char(
        "Setup Password (Terminal)",
        help="Password needed to introduce new Parameters (SSID, Odoo URL) during"
