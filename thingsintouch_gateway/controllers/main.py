...
display_messages = ["card_registered", "too_little_time_between_clockings"]
...
keys_defined_in_device = (
    factory_settings + defined_on_device_setup + defined_on_ack_from_device
)
...
class ThingsRasGate(http.Controller):
    ...
    def ModuleVersion(self, **kwargs):
        ...
        try:
            ...
            if "Please" in question:
                answer[
                    "version"
                ] = "11.0.5.0.210910"  # version of this things module as in __manifest__.py
            else:
                answer["error"] = "Wrong question"
                answer["version"] = None
                _logger.error(
                    "someone is asking the wrong question"
                    "(Get Module Version of ThingsRasGate Class)"
                )
        except Exception as e:
            _logger.info(
                "Get Module Version of ThingsRasGate Class - Exception {}".format(e)
            )
            answer["error"] = e
        _logger.info(
            "answer to Get Module Version of ThingsRasGate Class: {}".format(answer)
        )
        return answer

    @http.route(
        "/things/gates/ras/ack",
        type="json",
        auth="public",
        methods=["POST"],
        csrf=False,
    ...
    def AcknowdledgeRasGate(self, **kwargs):
        ...
        try:
            ...
            _logger.info(
                "the new gate request could not be dispatched - Exception {}".format(e)
            )
            answer["error"] = e
    ...
    def registerSingleton(self, card, timestamp_str, source):
        def getEmployeeID(card):
            ...
            employee_id = EmployeeModel.sudo().search(
                [("rfid_card_code", "=", card)], limit=1
            )
            if employee_id:
        ...
        if employee_id is None:
            ...
            result = attendanceModel.add_clocking(
                employee_id,
                timestamp_str,
    ...
    def registerClockings(self, routeFrom, data, answer):
        ...
        try:
            ...
            for c in data.get("clockings", []):
                ...
                # _logger.info(
                # 'registerClockings - card {} - timestamp_str {}'.format(card,timestamp_str))
    ...
    @http.route(
        "/things/gates/ras/incoming/<routeFrom>",
        type="json",
        auth="public",
        methods=["POST"],
        csrf=False,
    ...
    def messageFromGate(self, routeFrom, **kwargs):
        ...
        try:
            ...
            if productCategory == "RAS" and question == "RegisterClockings":
                answer = self.registerClockings(routeFrom, data, answer)

            if productCategory == "RAS" and question == "Reset":
                answer = self.resetSettings(routeFrom, answer)
        except Exception as e:
            _logger.info(
                "Message from Odoo To Gate could not be dispatched - Exception {}".format(
                    e
                )
            )
            answer["error"] = e
    ...
    @http.route(
        "/things/gates/ras/outgoing/<routeTo>",
        type="json",
        auth="public",
        methods=["POST"],
        csrf=False,
    ...
    def messageToGate(self, routeTo, **kwargs):
        ...
        try:
            ...
            if productCategory == "RAS" and question == "Routine":
